...
class VmtsProdi extends Component
{
    public $surveyTitle;
    ...
    {
        $survey = SurveyTypes::where('name', 'VMTSTIF')
            ->with([
    ...
    public function submit()
    {
        if ($this->respondentCategory != "mahasiswa" && $this->respondentCategory != "dosen" && $this->respondentCategory != "tendik") {
            $this->redirect(route('survey.tif'));
        }
        ...
        $this->redirect(route('survey.tif.submitted'));
    }

    public function render()
    {
        
        return view('livewire.home.survey.vmts-prodi');
    }
}
