<div class="max-w-2xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-white rounded-2xl shadow-lg p-6 sm:p-8 mb-8">
        <div class="mb-8">
            <h1 class="text-2xl md:text-3xl font-bold text-navy-800 mb-2">{{ $surveyTitle ?? 'Survey VMTS Prodi Teknik Informatika' }}</h1>
        ...
        <form wire:submit.prevent="submit" class="space-y-8">
            ...
            @if (count($sections ?? []) > 0)
                @foreach ($sections as $secIdx => $section)
                    <div class="mb-8">
                        <h2 class="text-lg font-bold text-navy-700 mb-2">
                        ...
                        <div class="space-y-6">
                            ...
                            @foreach ($questions as $q)
                                @if ($q['survey_section_id'] == $section['id'])
                                    <div class="bg-gray-50 rounded-xl p-4 border border-gray-200 relative">
                                        <label class="block text-base font-medium text-gray-800 mb-4">
                                            <span
                                                class="w-7 h-7 flex justify-center items-center rounded-full bg-navy-100 text-navy-700 font-semibold mr-2 text-center align-middle absolute -top-2 -left-2">{{ $qNum++ }}</span>
                                            {{ $q['question_text'] }}
                                            @if ($q['is_required'])
                                                <span class="text-red-500">*</span>
                                            @endif
