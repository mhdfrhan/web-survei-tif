<div class="max-w-2xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-white rounded-2xl shadow-lg p-8 mb-8">
        <div class="mb-8">
            <h1 class="text-2xl md:text-3xl font-bold text-navy-800 mb-2">{{ $surveyTitle ?? 'Survey Tenaga Pendidik' }}</h1>
                            ...
                            <div class="bg-gray-50 rounded-xl p-4 border border-gray-200 relative">
                                ...
                                @else
                                    <input type="text" wire:model="answers.{{ $q['id'] }}"
                                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-navy-500 focus:border-navy-500" autocomplete="off">
                                @endif
                                @error('answers.' . $q['id'])
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
