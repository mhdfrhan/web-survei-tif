<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-navy-800">Survey Mahasiswa Configuration</h1>
                <p class="text-navy-600 mt-2">Kelola survey pengukuran pemahaman Mahasiswa Fakultas Ilmu Komputer</p>
            </div>
        ...
        @if ($activeTab === 'fields')
            <div class="p-8">
                <div class="flex items-center justify-between mb-6">
                    ...
                    <x-primary-button wire:click="openFormFieldModal" x-data="" x-on:click.prevent="$dispatch('open-modal', 'add-form-field-modal')"
            ...
            <div class="p-8">
                <div class="flex items-center justify-between mb-6">
                    ...
                    <x-primary-button wire:click="openQuestionModal" x-data=""
                        x-on:click.prevent="$dispatch('open-modal', 'add-question-modal')" class="flex items-center space-x-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                        </svg>
                        ...
                        @foreach ($questions as $index => $question)
                            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                                <div class="flex items-start justify-between">
                                    <div class="flex-1">
                                        <h3 class="font-medium text-navy-800 mb-2">{{ $question['question_text'] }}
                                        </h3>
                                    ...
                                    <div class="flex space-x-2">
                                        ...
                                        <button wire:click="deleteQuestion({{ $index }})"
                                            class="text-red-600 hover:text-red-800 p-1">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <p class="text-gray-500">Belum ada questions. Tambahkan pertanyaan untuk mulai membuat survey.
                        </p>
                    </div>
                @endif
            </div>
            ...
            <div class="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <div class="p-6 space-y-4">
                            ...
                            <div class="space-y-2">
                                @foreach ($newFormField['field_options'] as $optIndex => $option)
                                    <div class="flex items-center space-x-2">
                                        <input type="text"
                                            wire:model="newFormField.field_options.{{ $optIndex }}"
                                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-navy-500 focus:border-navy-500"
                    ...
                    <div class="flex justify-end space-x-2 pt-4">
                        <button type="button" wire:click="closeFormFieldModal"
                            class="px-4 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-100">Batal</button>
                        <x-primary-button type="button" wire:click="saveFormField">Simpan
                            Field</x-primary-button>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        @foreach ($newQuestion['options'] as $optIndex => $option)
                            <div class="flex items-center space-x-2">
                                <input type="text" wire:model="newQuestion.options.{{ $optIndex }}"
                                    class="w-full px-2 py-1 border rounded" placeholder="Pilihan {{ $optIndex + 1 }}">
                                <button type="button" wire:click="removeMultipleChoiceOption({{ $optIndex }})"
                                    class="text-red-500 hover:text-red-700">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
